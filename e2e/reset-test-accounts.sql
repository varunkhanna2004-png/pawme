-- DEV ONLY. Puts the test accounts (ana / ben / dana @pawme.test) back to a clean
...
with t as (select id from auth.users where lower(email) in ('ana@pawme.test', 'ben@pawme.test', 'dana@pawme.test')),
ban as (delete from public.banned_identities returning 1),
